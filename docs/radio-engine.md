...
Ubuntu 24.04 distribution packages supply Liquidsoap (`2.2.4-1` on this server). Icecast uses Xiph's signed Ubuntu 24.04 repository, restricted to the `2.5.*` series (`2.5.0-1` installed on 2026-09-17). The installer configures a repository-specific signing key and package preferences, and validates the Liquidsoap script with `liquidsoap --check`. No OPAM build is required. See [Icecast 2.5 upgrade](icecast-upgrade.md) for verification and rollback. A fresh VM must still validate the installation path.
...
Icecast 2.5's real listener socket trusts a virtual proxy socket whose client address is `127.0.0.1`. Nginx stream snippets replace `X-Forwarded-For` with `$remote_addr`, so a public caller cannot inject its own reported client address. The proxy only exposes exact stream paths; private administration and client lists remain on the loopback backend. If another proxy/CDN is introduced, separately configure Nginx's trusted real-IP boundary before relying on audience geography.
