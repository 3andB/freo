...
Browser and real-engine tests require local socket access and use temporary databases/media. Explicitly disabling the installation's microphone input keeps these tests isolated. The Cue engine test records audio and checks that both generated song frequencies actually aired.
