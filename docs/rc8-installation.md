# RC8 test kit: Ubuntu 24.04 x86_64

Version: **0.3.0-rc.8**. This is a signed private test candidate, not an accepted
...
The publisher supplies `freo-v0.3.0-rc.8-test-kit.tar` plus
...
tar -xf freo-v0.3.0-rc.8-test-kit.tar
cd freo-test-kit
```
...
gpgv --keyring "$PWD/publisher.gpg" \
  freo-v0.3.0-rc.8.tar.gz.asc freo-v0.3.0-rc.8.tar.gz
mkdir freo-v0.3.0-rc.8
tar -xzf freo-v0.3.0-rc.8.tar.gz -C freo-v0.3.0-rc.8
cd freo-v0.3.0-rc.8
```
...
```bash
sudo env FREO_DOMAIN=rc8.example.com FREO_ENABLE_HTTPS=1 \
...
The version must be `0.3.0-rc.8`. There are no seeded demo stations or songs.
...
start broadcasting. Follow `rc8-acceptance.md` in the test kit.
...
Freo Live discovers a stable GitHub Release. RC8 must not claim to be stable or
...
installer over existing Freo state. Historical candidate upgrade rehearsals are
