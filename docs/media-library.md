...
The import page accepts WAV, M4A, MP3 and FLAC files, multiple selections, complete albums, or a browser folder. Browser uploads first become durable review drafts; choosing Import creates independent ingest jobs. Invalid or duplicate files do not discard the rest of a batch. See [the review workspace and rollout](music-importer-testing.md). Requests allow 128 MiB per file and 512 MiB total by default; `MAX_MEDIA_UPLOAD_BYTES` and `MAX_MEDIA_BATCH_BYTES` configure those bounded limits.
...
Choose files or a folder, or drop either onto Import music. The screen lists selections and uploads each song independently with per-file progress and results. Folder paths are reduced to file basenames for storage validation. Processing status is polled from a station-scoped authenticated endpoint; files stay outside the library until the user imports them, then enable after successful processing unless Keep disabled was chosen. Failed uploads can be retried. Nginx permits the application's maximum configurable batch size; Flask enforces the configured body and per-file limits. Imaging retains its native form submission and result page.
