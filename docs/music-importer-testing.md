...
- Full repository run: **641 passed, 32 skipped, 3 failed**. Two failures were legacy migration tests incorrectly upgrading to the latest revision; they now target their own migration and pass. The remaining `test_workspace_browser.py::test_calendar_create_edit_and_mobile_layout` failure also reproduced alone: Selenium could not click a scheduling timeline item at `(386, 1057)`. It remains unresolved in the preexisting scheduling work. The full suite is therefore **not entirely green**.
- JavaScript syntax and `git diff --check`: passed.

The user approved production backup, migration, commit, push and web/ingest restart on 2026-09-18. Rollout uses the deployment steps above. The starting database revision is `ab31e76f209d`; the target is `e92b740a613f`. The earlier backup attempt was rejected before execution; no production changes were made during the testing phase.
