...
The DJ Booth stores an explicit operator mode, idempotent manual selection decisions, fixed skip/takeover intents, assignable station cart positions, and a sanitized worker-observed queue snapshot. The existing automation worker validates approved media and performs only fixed allowlisted queue/advance operations. Takeover is bound to the confirmed current request and becomes terminal on failure, preventing a destructive retry against a later item. The web process remains unable to access Liquidsoap sockets. See [Live Assist](live-assist.md).
