...
The station **DJ Booth** centers the observed on-air item, real Up Next queue, in-booth music/category browser, eight hot carts, four identity positions, and illuminated Auto / Live Assist / Live mode controls. Songs may be dragged to Queue End or dropped on Now Playing for a deliberate worker-mediated takeover. Stereo meters sample the station monitor only after the operator enables it. Unsupported queue reorder/remove, arbitrary fade, and on-air pause remain visibly unavailable. Engineering and reconciliation state sits in a collapsed status drawer. See [Live Assist](live-assist.md).
