...
The station **DJ Booth** provides a focused Auto layout and paired-deck DJ Booth layout. Its pointer-driven song drag supports Queue End, a real queued Deck B, and On Air takeover; both decks also have searchable Cue Song pickers. Every one of the eight hot carts and four identity positions has direct assignment controls plus drag assignment. The header shows the current song over its artist and animates a dense multicolor meter with fading peaks from worker-observed Liquidsoap RMS. The Monitor meter samples optional browser audio, and an elapsed ring wraps the playing platter. Submitted queue reorder/remove remains unavailable; engineering and reconciliation state sits in a collapsed status drawer. See [Live Assist](live-assist.md).
