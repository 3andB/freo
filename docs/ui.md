...
After sign-in, `/admin` is the operational overview. `/admin/stations` lists all managed stations, including stopped stations, and `/admin/stations/<slug>` opens a deeper station view with pending approved requests. The sidebar has authenticated Media management and read-only Categories, Rotations, Clocks, Schedule, History, and System pages. The station picker keeps those pages station-scoped. The old `/dashboard/<slug>` URL redirects to the station detail page.
