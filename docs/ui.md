...
The station **DJ Booth** provides separate Auto, DJ Booth, and paired-deck Live layouts. Songs can be dragged to Queue End, persistent Cue, or On Air takeover. Every one of the eight hot carts and four identity positions has direct assignment controls plus drag assignment. Program and Cue meter banks sample real monitored audio, and an elapsed ring wraps the playing platter. Submitted queue reorder/remove remains unavailable; engineering and reconciliation state sits in a collapsed status drawer. See [Live Assist](live-assist.md).
