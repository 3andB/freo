...
def artist_for(station_id, name):
    ...
    if not row:
        try:
            ...
            row=Artist.query.filter_by(station_id=station_id,normalized_name=key).one()
    return row
...
def album_for(station_id, artist, title, album_artist='', year=None, genre=''):
    ...
    if not row:
        try:
            ...
            row=Album.query.filter_by(station_id=station_id,artist_id=artist.id,normalized_title=key).one()
    else:
...
def organize_song(song, tags=None):
    ...
    album_artist=normalize(tags.get('album_artist') or tags.get('albumartist') or song.album_artist,200,'')
    ...
    album_owner=artist_for(song.station_id,album_artist) if album_artist else artist
    album=album_for(song.station_id,album_owner,song.album,album_artist,year,genre)
