...
from app.services.playout_queue import (_command, push_decision, socket_identity,
                                       queued_ids, active_ids, request_decision_id)
...
def process_transition(station, reader):
    ...
    try:
        ...
        if response=='APPLIED':
            reader.collect(station.slug)
            if decision.status!='started':return True
            ...
            command.state='APPLIED';command.completed_at=datetime.now(timezone.utc)
        ...
        if command.state=='PENDING' and (datetime.now(timezone.utc)-created).total_seconds()>30:
            command.state='FAILED';command.error='Engine is unavailable or needs the scheduling update. Current mode retained.'
        else:
