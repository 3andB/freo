...
def finalize_log(log):
    ...
    for p in log.placements: by_stop[p.stopset].append(p)
...
def create_makegood(original,stopset):
    if original.status not in ('MISSED','FAILED'): raise ValueError('Only missed or failed placements need a makegood')
    if stopset.station_id!=original.station_id: raise ValueError('Makegood stopset belongs to another station')
    existing=TrafficPlacement.query.filter_by(traffic_log_id=original.traffic_log_id,traffic_stopset_id=stopset.id).all()
    used=sum(p.creative.imaging_asset.duration_ms for p in existing)/1000
    duration=original.creative.imaging_asset.duration_ms/1000
    fixed=sum(i.imaging_asset.duration_ms for i in stopset.template_items if i.imaging_asset)/1000
    if fixed+used+duration>stopset.capacity_seconds: raise ValueError('Makegood does not fit stopset capacity')
    row=TrafficPlacement(traffic_log_id=original.traffic_log_id,station_id=original.station_id,traffic_stopset_id=stopset.id,scheduled_for_utc=_instant(original.traffic_log.station,original.traffic_log.log_date,stopset.local_time),campaign_id=original.campaign_id,commercial_creative_id=original.commercial_creative_id,position=len(existing)+1,status='PLANNED',is_makegood=True,makegood_for_id=original.id,advertiser_name=original.advertiser_name,campaign_name=original.campaign_name,creative_name=original.creative_name,creative_code=original.creative_code);db.session.add(row);db.session.commit();return row
