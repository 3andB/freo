(() => {
  ...
  if(!root)return;
  const panel=root.querySelector('.mic-console'), $=id=>document.getElementById('mic-'+id);
  ...
  scope.listen($('connect'),'click',async()=>{
    ...
    try{
      ...
      await devices();
      stream.getAudioTracks()[0].addEventListener('ended',()=>{message('Audio input disconnected. Returning to AUTO.',true);disconnect();});
      ...
      const config=await api('config');pc=new RTCPeerConnection(config);pc.addTrack(output.stream.getAudioTracks()[0],output.stream);
      const connection=pc;
