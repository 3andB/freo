(() => {
  ...
  [document.querySelector('.admin-topbar'),root.querySelector('.room-search'),root.querySelector('.selection-toolbar'),root.querySelector('.room-library > header'),$('music-player')].filter(Boolean).forEach(node=>layoutObserver.observe(node));
  ...
  $('share-selected').onclick=async()=>{if(await post('share-all',{songs:[...selected]}))await load();};
