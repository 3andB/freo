(() => {
  ...
  const songDialog=document.getElementById('song-picker-dialog'),songSearch=document.getElementById('song-picker-search'),songResults=document.getElementById('song-picker-results');let searchTimer;
  const searchSongs=async()=>{const response=await fetch(`${root.dataset.songSearchUrl}?q=${encodeURIComponent(songSearch.value)}`,{credentials:'same-origin',cache:'no-store'});const songs=await response.json();songResults.replaceChildren();for(const song of songs){const button=document.createElement('button');button.type='button';button.innerHTML='<b></b><span></span><small></small>';button.querySelector('b').textContent=song.title;button.querySelector('span').textContent=song.artist;button.querySelector('small').textContent=`${song.album||'Single'}${song.bpm?` · ${Math.round(song.bpm)} BPM`:''}`;button.addEventListener('click',()=>post('cue',{identifier:song.uuid}));songResults.append(button);}if(!songs.length)songResults.textContent='No enabled songs found.';};
  document.querySelectorAll('.cue-picker-button').forEach(button=>button.addEventListener('click',()=>{songDialog.showModal();songSearch.focus();searchSongs();}));
  songSearch.addEventListener('input',()=>{clearTimeout(searchTimer);searchTimer=setTimeout(searchSongs,180);});
  ...
  async function refresh(){try{const response=await fetch(root.dataset.statusUrl,{credentials:'same-origin',cache:'no-store'});if(!response.ok)throw new Error();state=await response.json();root.dataset.mode=state.mode;root.className=`dj-booth booth-mode-${state.mode.toLowerCase().replace('_','-')}`;text('led-detail',state.mode.replace('_',' '));text('live-mode',state.mode);text('live-clock',state.clock||'None');text('auto-clock',state.clock||'No active clock');text('live-transition',state.next_transition||'None');text('live-playout',state.playout_error||'Connected');text('live-fallback',state.fallback);text('next-event-name',state.next_event?.name||'None');text('current-source',state.current?.source||'FALLBACK');const current=state.current;const activeDeck=current?.deck||'A';document.querySelector('.onair-deck').classList.toggle('active',!!current&&activeDeck==='A');document.querySelector('.cue-deck').classList.toggle('active',!!current&&activeDeck==='B');text('deck-a-label',current&&activeDeck==='A'?'LIVE':'A');text('deck-b-label',current&&activeDeck==='B'?'LIVE':'B');const rms=Number.isFinite(state.program_rms)?Math.min(1,state.program_rms*2.8):0;programTarget=rms;document.getElementById('program-left').value=rms;document.getElementById('program-right').value=rms;if(current?.decision_id!==morphDecision){morphDecision=current?.decision_id;text('morph-text',current?.title||'ON AIR');text('morph-artist',current?.artist||'FREO');}document.querySelector('#live-current h2').textContent=current?.title||'Fallback / awaiting confirmation';document.querySelector('#live-current p').textContent=current?.artist||'Live broadcast';text('now-album',current?.album||'Live broadcast');text('auto-title',current?.title||'Fallback / awaiting confirmation');text('auto-artist',current?.artist||'Live broadcast');text('auto-category',current?.category||'No category');text('fact-category',current?.category||'NO CATEGORY');text('fact-bpm',current?.bpm?`${Math.round(current.bpm)} BPM`:'BPM —');text('fact-year',current?.year||'YEAR —');text('fact-lufs',current?.loudness_lufs?`${current.loudness_lufs.toFixed(1)} LUFS`:'LUFS —');text('queue-count',state.queue.length);fillQueue(state.queue);document.querySelectorAll('.current-decision').forEach(input=>input.value=current?.decision_id||'');document.querySelectorAll('.current-control button').forEach(button=>button.disabled=!current);document.querySelectorAll('.mode-button').forEach(button=>button.classList.toggle('active',button.dataset.mode===state.mode));const deckB=activeDeck==='B'?current:state.cue;text('cue-title',deckB?.title||'NOTHING CUED');text('cue-artist',deckB?.artist||'Drag a song here before taking control');text('cue-album',deckB?.album||'Cue deck is empty');document.getElementById('cue-warning').hidden=!!deckB;timing();}catch(_){text('live-playout','Status unavailable');}}
