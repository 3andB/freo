(() => {
  ...
  async function probePublicStream() {
    ...
    if (!/^\/(stream|listen)\/[a-z0-9][a-z0-9-]*$/.test(path)) {
    ...
    try {
      const response = await scope.fetch(path, {cache: 'no-store', signal: controller.signal});
  ...
  probePublicStream();
  if (document.querySelector('.admin-stream-probe')) scope.interval(probePublicStream, 60000);
  ...
  scope.interval(refreshNow, 5000);
  scope.interval(refreshRuntime, 15000);
  scope.interval(refreshProgramming, 45000);
})();
