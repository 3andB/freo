...
def test_sound_room_preview_tag_drag_category_editor_notes_and_undo(booth):
    ...
    wait_text(driver,'.song-row-chips','HOT')
    driver.find_element(By.ID,'room-undo').click()
    WebDriverWait(driver,8,ignored_exceptions=(StaleElementReferenceException,)).until(lambda d:'HOT' not in d.find_element(By.CSS_SELECTOR,'.song-row-chips').text)
    ...
    driver.find_element(By.CSS_SELECTOR,'.category-membership').click()
    wait_text(driver,'#room-songs','No songs match')
    driver.find_element(By.XPATH,"//button[text()='Browse songs to add']").click()
    wait_text(driver,'#room-songs','Verified Test Track')
    driver.find_element(By.CSS_SELECTOR,'.category-membership').click()
    wait_text(driver,'.category-membership','Remove from category')
