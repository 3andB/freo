...
def wait_for_appearance(driver, theme):
    # A refresh can expose the replacement document before its theme script
    # and DOMContentLoaded control synchronization have finished on CI.
    WebDriverWait(driver, 10).until(lambda d: d.execute_script(
        "return document.documentElement.dataset.theme === arguments[0] && "
        "document.querySelector('[data-appearance=\"' + arguments[0] + '\"]')"
        "?.getAttribute('aria-pressed') === 'true'", theme))
...
def test_appearance_persistence_navigation_and_audio(booth):
    ...
    assert driver.execute_script('return originalMonitor === FreoMonitor.audio && !FreoMonitor.audio.paused')
    driver.refresh()
    wait_for_appearance(driver, 'night')
...
def test_legacy_preference_storage_and_unsaved_input(booth):
    ...
    driver.refresh()
    wait_for_appearance(driver, 'day')
